#!/usr/bin/env python3
"""
ICLR 2026 世界模型技术线索推荐 PPT 生成脚本（带示意图）
"""
...
def add_paper_slide(prs, paper_info):
    ...
    innovation_box = slide.shapes.add_textbox(Inches(4.5), Inches(1.75), Inches(8.3), Inches(1.2))
    ...
    # 方法示意图
    fig_title = slide.shapes.add_textbox(Inches(4.5), Inches(3.05), Inches(8.3), Inches(0.35))
    ...
    if paper_info.get('figure_path') and os.path.exists(paper_info['figure_path']):
        try:
            ...
            fig_box = slide.shapes.add_shape(
                MSO_SHAPE.RECTANGLE, Inches(4.5), Inches(3.4), Inches(8.3), Inches(1.6)
            )
            fig_box.fill.solid()
...
# 保存
output_path = '/Users/gesong/.openclaw/workspace/ICLR2026-世界模型技术线索推荐-完整版.pptx'
